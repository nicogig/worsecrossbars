...
def require_webhook ():
    ...
    with open(str(config.working_dir.joinpath("utilities", "config", "msteams.json")), 'w') as outfile:
        json.dump(jsonified_webhook, outfile)

def check_webhook_presence ():
    global webhook_present, webhook_url
    if not os.path.exists(config.working_dir.joinpath("utilities", "config", "msteams.json")):
        require_webhook()
        check_webhook_presence()
    else:
        with open(str(config.working_dir.joinpath("utilities", "config", "msteams.json"))) as json_file:
